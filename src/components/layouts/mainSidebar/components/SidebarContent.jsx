...
import { faChevronDown, faCirclePlus, faCompass, faEllipsis, faEnvelope, faHouse, faPaperPlane, faStarOfLife, faUserAstronaut, faUserGroup, faUserPlus } from '@fortawesome/free-solid-svg-icons';
...
const SidebarContent = () => {
    const {isFinding} = useContext(SidebarContext);
    return <>
        <div 
        ...
        >
            {navItems.map(item => (
                ...
                >
                    <FontAwesomeIcon icon={item.icon} className={cx('sidebar-content-icon')} />
                    {!isFinding &&
        ...
        <div className={cx('sidebar-follower')}>
            ...
            {userList.map(user => (
                ...
                > 
                    <FontAwesomeIcon icon={faUserAstronaut} className={cx('sidebar-content-icon')} />
